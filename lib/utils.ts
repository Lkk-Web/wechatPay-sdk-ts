...
//生成原始数据签名
export function getSignature(method: "POST" | 'GET', privateKey: string, pathname: string, timeStamp: number | string, nonceStr: string, body?: object | string) {
	if (body) {
	...
	return sha256WithRsaSign(data, privateKey)
}
//签名算法，加密
export function sha256WithRsaSign(plaintext: string, privateKey: string) {
	return createSign('RSA-SHA256').update(plaintext).sign(privateKey, 'base64')
}
//验证微信平台响应签名
...
export function aesGcmDecrypt(nonce: string, associatedData: string, ciphertext: string, apiv3: string) {
	try {
		...
		let decipherIv = createDecipheriv('aes-256-gcm', apiv3, nonce)
